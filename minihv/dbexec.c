    ...
    ACPI_DB_METHOD_INFO     *Info)
{
    ...
    if ((Info->Name[0] != '\\') &&
        (Info->Name[0] != '/'))
    {
        if (AcpiUtSafeMstrcat (Info->Pathname, sizeof (Info->Pathname),
            AcpiGbl_DbScopeBuf))
        {
    ...
    UINT32                  Flags)
{
    ...
    {
        NameString = ACPI_ALLOCATE (Mstrlen (Name) + 1);
        if (!NameString)
        {
            return;
        }

        Mmemset (&AcpiGbl_DbMethodInfo, 0, sizeof (ACPI_DB_METHOD_INFO));

        Mstrcpy (NameString, Name);
    ...
    char                    *MethodNameArg)
{
    ...
    Mmemset (AcpiGbl_DbMethodInfo.Threads, 0, Size);
