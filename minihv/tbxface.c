    ...
    BOOLEAN                 AllowResize)
{
     ...
     * if requested
     */
    LOG_INFO("Setup the Root Table array and allocate table array");
    if (!InitialTableArray)
    {
        LOG_INFO("Calling AcpiAllocateRootTable...");
    ...
    LOG_INFO("Calling AcpiOsGetRootPointer...");
    RsdpAddress = AcpiOsGetRootPointer ();
    if (!RsdpAddress)
    {
        LOG_ERROR("Rsdp Address not found");
     ...
     */
    LOG_INFO("Calling AcpiTbParseRootTable...");
    ...
    void)
{
    ...
    LOG_INFO("AcpiReallocateRootTable called");
    /*
